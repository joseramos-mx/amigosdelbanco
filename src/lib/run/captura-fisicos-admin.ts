...
export class FolioSinVendedor extends Error {
    constructor() {
        super("Este folio no tiene un vendedor asignado.");
        this.name = "FolioSinVendedor";
    }
}
...
export async function capturarFisico(
    datos: DatosCaptura,
    actorId: string // id del admin que está haciendo la captura (para auditoría), NO del vendedor
    ...
    `;
        ...
        // Antes se exigía que quien captura sea el mismo vendedor dueño del
        // folio. Ahora captura el admin, así que ya no se valida identidad
        // aquí: el vendedor_id que ya trae la orden (asignado desde que se
        // generó el folio físico) es el que se respeta y se acredita.
        if (!orden.vendedor_id) throw new FolioSinVendedor();
...
export async function buscarVendedorPorFolio(
    folio: string
): Promise<{ nombre: string } | null> {
    const sql = db();

    // Solo nos interesan folios que todavía están pendientes de captura:
    // si ya fue pagado o no existe, no hay nada que mostrar.
    const [orden] = await sql<{ vendedor_id: string | null }[]>`
    select vendedor_id
      from public.orden
     where folio = ${folio}
       and estado = 'pendiente'
  `;
    ...
    if (!orden || !orden.vendedor_id) return null;

    const [vendedor] = await sql<{ nombre: string }[]>`
    select nombre from public.usuario_rol where id = ${orden.vendedor_id}
  `;
